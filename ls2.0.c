...
void display_R(char *path)             //-R参数
{
    ...
    DIR *dir;
    struct dirent *ptr;
    char allname[251][261],name[251][261],a[251];
    int i,j,k,len,count;
    ...
    if(S_ISDIR(buf.st_mode))    //为一个目录，还有文件或子目录
    {
        ...
        for(i=0;i<count;i++ )    
        {
            for(j=0,k=0;k<strlen(allname[i]);k++)
            {
                if(allname[i][k] == '/')
                {
                    j=0;
...
void display_dir(int flag_param,char *path)             
{
    ...
    for(i = 0;i<count-1;i++)
        ...
        for(i=0;i<count;i++)
        {
            for(k=0,j=0;j<strlen(fullname[i]);j++)
            {
                if(fullname[i][j] == '/')
                {
                    k=0;
